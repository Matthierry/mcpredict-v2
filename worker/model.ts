...
const COL = { marketId: 9, date: 14, time: 15, home: 16, away: 17, league: 60, homeProbability: 44, drawProbability: 45, awayProbability: 46, underProbability: 61, overProbability: 62 } as const;
...
export function parseModelCsv(csv: string): ModelFixture[] {
  ...
  for (const row of result.data) {
    ...
    fixtures.push({ marketId, kickoffUtc, homeTeam, awayTeam, league, predictions: probabilities.filter((entry): entry is ["1x2" | "ou25", Outcome, number] => entry[2] != null).map(([market, outcome, probability]) => ({ market, outcome, probability })) });
  }
  return fixtures;
}
